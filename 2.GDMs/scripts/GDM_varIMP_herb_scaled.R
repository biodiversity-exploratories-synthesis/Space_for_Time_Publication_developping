...
##### data prep
#######
# avoid that any beta diversity value is bigger or smaller than 1 and 0 respectively
# should not be but sometimes it deviated by very small values and that made the GDM stop

pwise_time_ayrs_in$hcqn0dis[pwise_time_ayrs_in$hcqn0dis > 0.999] <- 0.999
pwise_time_ayrs_in$hcqn0dis[pwise_time_ayrs_in$hcqn0dis < 0.001] <- 0.001

pwise_space_in$hcqn0dis[pwise_space_in$hcqn0dis > 0.999] <- 0.999
pwise_space_in$hcqn0dis[pwise_space_in$hcqn0dis < 0.001] <- 0.001

pwise_time_ayrs_in$hcqn1dis[pwise_time_ayrs_in$hcqn1dis > 0.999] <- 0.999
pwise_time_ayrs_in$hcqn1dis[pwise_time_ayrs_in$hcqn1dis < 0.001] <- 0.001

pwise_space_in$hcqn1dis[pwise_space_in$hcqn1dis > 0.999] <- 0.999
pwise_space_in$hcqn1dis[pwise_space_in$hcqn1dis < 0.001] <- 0.001

pwise_time_ayrs_in$hcqn2dis[pwise_time_ayrs_in$hcqn2dis > 0.999] <- 0.999
pwise_time_ayrs_in$hcqn2dis[pwise_time_ayrs_in$hcqn2dis < 0.001] <- 0.001

pwise_space_in$hcqn2dis[pwise_space_in$hcqn2dis > 0.999] <- 0.999
pwise_space_in$hcqn2dis[pwise_space_in$hcqn2dis < 0.001] <- 0.001

pwise_time_ayrs_in$hcqn3dis[pwise_time_ayrs_in$hcqn3dis > 0.999] <- 0.999
pwise_time_ayrs_in$hcqn3dis[pwise_time_ayrs_in$hcqn3dis < 0.001] <- 0.001

pwise_space_in$hcqn3dis[pwise_space_in$hcqn3dis > 0.999] <- 0.999
pwise_space_in$hcqn3dis[pwise_space_in$hcqn3dis < 0.001] <- 0.001

pwise_time_ayrs_in$hcqn4dis[pwise_time_ayrs_in$hcqn4dis > 0.999] <- 0.999
pwise_time_ayrs_in$hcqn4dis[pwise_time_ayrs_in$hcqn4dis < 0.001] <- 0.001

pwise_space_in$hcqn4dis[pwise_space_in$hcqn4dis > 0.999] <- 0.999
pwise_space_in$hcqn4dis[pwise_space_in$hcqn4dis < 0.001] <- 0.001
...
for (i in 1:4) {
  ...
  # nrow(dat) == (choose((nplots + 2 - 1), 2) - nplots) * nyears # number of rows not more than expected
...
for (i in 1:4) {
  ...
  nyears <- length(unique(c(dat$s1.Year, dat$s2.Year)))
  length(unique(c(dat$s1.LU, dat$s2.LU))) <= nyears * nplots
  nrow(dat) <= (choose((nplots + 2 - 1), 2) - nplots) * nyears # number of rows as expected or less
  # here : not every year has same number of plots
  nrow(unique(rbindlist(list(
    unique(dat[which(dat$s1.Year == "2008"), c("s1.xCoord", "s1.yCoord", "s1.LU")]),
    unique(dat[which(dat$s2.Year == "2008"), c("s2.xCoord", "s2.yCoord", "s2.LU")])
  ),
  use.names = F
  ))) # 139 plots in 2008 (more in other years)
  test <- unique(rbindlist(list(
    unique(dat[which(dat$s1.Year == "2008"), c("s1.xCoord", "s1.yCoord", "s1.LU", "s1.Year")]),
    unique(dat[which(dat$s2.Year == "2008"), c("s2.xCoord", "s2.yCoord", "s2.LU", "s2.Year")])
  ),
  use.names = F
  ))
...
for (i in 1:4) {
  ...
  nrow(dat) <= (choose((nplots + 2 - 1), 2) - nplots) * nyears # number of rows as expected or less
  nrow(unique(rbindlist(list(
    unique(dat[which(dat$s1.Year == "2010"), c("s1.xCoord", "s1.yCoord", "s1.LU")]),
    unique(dat[which(dat$s2.Year == "2010"), c("s2.xCoord", "s2.yCoord", "s2.LU")])
  ),
  use.names = F
  ))) # 148 plots in 2008
  test <- unique(rbindlist(list(
    unique(dat[which(dat$s1.Year == "2010"), c("s1.xCoord", "s1.yCoord", "s1.LU", "s1.Year")]),
    unique(dat[which(dat$s2.Year == "2010"), c("s2.xCoord", "s2.yCoord", "s2.LU", "s2.Year")])
  ),
  use.names = F
  ))
  test <- test[with(test, order(s1.xCoord, s1.yCoord)), ]
...
for (i in 1:4) {
  ...
  nyears <- length(unique(c(dat$s1.Year, dat$s2.Year)))
  length(unique(c(dat$s1.LU, dat$s2.LU))) <= nyears * nplots
  nrow(dat) <= (choose((nplots + 2 - 1), 2) - nplots) * nyears # number of rows max. as expected
  nrow(unique(rbindlist(list(
    unique(dat[which(dat$s1.Year == "2008"), c("s1.xCoord", "s1.yCoord", "s1.LU")]),
    unique(dat[which(dat$s2.Year == "2008"), c("s2.xCoord", "s2.yCoord", "s2.LU")])
  ),
...
for (i in 1:4) {
  ...
  # length(unique(c(dat$s1.LU, dat$s2.LU))) <= nyears * nplots # known problem : too many LUI values
  # length(unique(c(dat$distance, dat$distance)))
  # # 5943 rows
  # nrow(dat) <= (choose((nyears + 2 - 1), 2) - nyears) * nplots
  # ## check dataset end

  dat <- dat[complete.cases(dat), ] # already only complete cases included
...
for (i in 1:4) {
  ...
  # length(unique(c(paste(dat$s1.xCoord, dat$s1.yCoord), paste(dat$s2.xCoord, dat$s2.yCoord)))) # 150 plots (other method, same result)
  # nyears <- length(unique(c(dat$s1.Year, dat$s2.Year))) # 10 years
  # length(unique(c(dat$s1.LU, dat$s2.LU))) <= nyears * nplots # expected number or LUI values
  # length(unique(c(dat$distance, dat$distance))) # 5603 rows
  # nrow(dat) <= (choose((nyears + 2 - 1), 2) - nyears) * nplots # not more than expected rows
  # ## check dataset end
