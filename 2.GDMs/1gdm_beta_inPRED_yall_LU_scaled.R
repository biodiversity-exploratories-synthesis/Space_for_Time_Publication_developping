# # # # # # # # # # # # # # #  # # # # #
#                                      #
#              FIT GDMS                # 
#  BETA DIVERSITY PREDATORS TEMPORAL   # ----
#                                      #
# # # # # # # # # # # # # # #  # # # # #

# # # # # # # # # # # # # #
# CONTENT                  ----
# AIM : Fit GDM models of predators beta diversity, for temporal datasets.
# OUTPUT : 
#TODO


# # # # # # # # # # # # # #
# 0 - REQUIREMENTS              ----
#
# packages
# install.packages("gdm")
library(gdm)
library(vegan)
library(tidyverse)
library(ggplot2)
library(patchwork)

# # # # #
# 0.a. - DATA ----
#
#insects
#load("./pwise_time_ayrs_in_new.RData")
load("./sitepred_pwise_time_in_step3.RData")#new pwise_time with updated LUI residuals

#a0 - all insects
#pa0 - predators/secondary consumers
#ha0 - predbivores
#poa0 - predlinators
#da0  - predomposers


#TODO is there no part where the range is taken?

# # # # # # # # # # # # # #
# 2 - TEMPORAL DATASET              ----
#
# only predator insects
# create a data frame for the splines

splines_pred_yall_sc<- as.data.frame(matrix(nrow=1200, ncol=16))
colnames(splines_pred_yall_sc)<- c("LUIx", "MOWx", "GRAx", "FERx", 
                                "COvLUIx","COvMOWx", "COvGRAx", "COvFERx",
                                "LUIy", "MOWy", "GRAy", "FERy", 
                                "COvLUIy","COvMOWy", "COvGRAy", "COvFERy")
splines_pred_yall_sc$beta_type<- rep(c("cqn0", "cqn1", "cqn2", "cqn3", "cqn4", "bsim"), each=200)

LU1<- c("LUI1res", "MOW1res", "GRA1res", "FER1res")
LU2<- c("LUI2res", "MOW2res", "GRA2res", "FER2res")

LU<- c("LUI", "MOW", "GRA", "FER")


# # # # #
# 2.0. -  b0 ----
# 
# b0 : beta diversity with Chao = 0
for(i in 1:4){
  
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pcqn0dis, 
                   rep(1, nrow(pwise_time)), 
                   HWG, HWG1, HWG, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]

  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)
  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[,i]<- spline$x[,1]
  splines_pred_yall_sc[,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[,i+8]<- spline$y[,1]
  splines_pred_yall_sc[,i+12]<- spline$y[,2]
}

remove(dat,gdmTab,  GDM)

# # # # #
# 2.1. -  b1 ----
# 
# b1 : beta diversity with Chao = 1
pwise_time$pcqn1dis[pwise_time$pcqn1dis>0.999]<- 0.999
pwise_time$pcqn1dis[pwise_time$pcqn1dis<0.001]<- 0.001

for(i in 1:4){
  
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pcqn1dis, 
                   rep(1, nrow(pwise_time)), 
                   HWG1, HWG1, HWG2, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]
  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)
  
  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[201:400,i]<- spline$x[,1]
  splines_pred_yall_sc[201:400,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[201:400,i+8]<- spline$y[,1]
  splines_pred_yall_sc[201:400,i+12]<- spline$y[,2]
}

remove(dat, gdmTab,  GDM)

# # # # #
# 2.2. -  b2 ----
# 
# b2 : beta diversity with Chao = 2
for(i in 1:4){
  
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pcqn2dis, 
                   rep(1, nrow(pwise_time)), 
                   HWG1, HWG1, HWG2, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]
  
  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)
  
  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[401:600,i]<- spline$x[,1]
  splines_pred_yall_sc[401:600,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[401:600,i+8]<- spline$y[,1]
  splines_pred_yall_sc[401:600,i+12]<- spline$y[,2]
}

remove(dat, gdmTab, GDM)


# # # # #
# 2.3. -  b3 ----
# 
# b3 : beta diversity with Chao = 3
for(i in 1:4){
  
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pcqn3dis, 
                   rep(1, nrow(pwise_time)), 
                   HWG1, HWG1, HWG2, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]
  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)

  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[601:800,i]<- spline$x[,1]
  splines_pred_yall_sc[601:800,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[601:800,i+8]<- spline$y[,1]
  splines_pred_yall_sc[601:800,i+12]<- spline$y[,2]
}

remove(dat, gdmTab, GDM)

# # # # #
# 2.4. -  b4 ----
# 
# b4 : beta diversity with Chao = 4
for(i in 1:4){
   
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pcqn4dis, 
                   rep(1, nrow(pwise_time)), 
                   HWG1, HWG1, HWG2, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]
  
  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)
  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[801:1000,i]<- spline$x[,1]
  splines_pred_yall_sc[801:1000,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[801:1000,i+8]<- spline$y[,1]
  splines_pred_yall_sc[801:1000,i+12]<- spline$y[,2]
}

remove(dat, gdmTab,  GDM)

# # # # #
# 2.5. -  bsim ----
# 
# bsim : beta diversity simpson
for(i in 1:4){
  
  attach(pwise_time)
  
  #year and LU as predictors
  dat<- data.frame(pbsim, 
                   rep(1, nrow(pwise_time)), 
                   HWG1, HWG1, HWG2, HWG2, 
                   pwise_time[,colnames(pwise_time)== LU1[i]], 
                   YR1, 
                   pwise_time[,colnames(pwise_time)== LU2[i]], 
                   YR2)
  
  colnames(dat)<- c("distance", "weights", 
                    "s1.xCoord", "s1.yCoord", "s2.xCoord", "s2.yCoord", 
                    "s1.LU","s1.Year", "s2.LU", "s2.Year")
  
  dat<- dat[complete.cases(dat),]
  
  
  detach(pwise_time)
  
  gdmTab<- formatsitepair(dat, 4, predData=dat)
  GDM<- gdm(gdmTab, geo=F)
  
  
  spline<- isplineExtract(GDM)
  splines_pred_yall_sc[1001:1200,i]<- spline$x[,1]
  splines_pred_yall_sc[1001:1200,i+4]<- spline$x[,2]
  
  splines_pred_yall_sc[1001:1200,i+8]<- spline$y[,1]
  splines_pred_yall_sc[1001:1200,i+12]<- spline$y[,2]
  
}

remove(dat, gdmTab, GDM)


save(splines_pred_yall_sc, file="splines_pred_yall_sc_new.RData")


# # # # #
# 2.5. -  PLOT ----
# 
load("./splines_pred_yall_sc_new.RData")#see prep code above

splines_pred_yall_sc$beta_type<- as.factor(splines_pred_yall_sc$beta_type)
levels(splines_pred_yall_sc$beta_type)

# LUI
p1<- ggplot(splines_pred_yall_sc, aes(x=LUIx, y=LUIy, colour=beta_type))+
  geom_line(size=1.5)+
  scale_y_sqrt(limits = c(0, 3), expand = c(0, 0))+
  scale_colour_hue(name="Type of Beta Diversity", labels=c("Turnover", "Chao 0",
                                                           "Chao 1", "Chao 2",
                                                           "Chao 3", "Chao 4"),l=50)+
  xlab("LUI (scaled to EP average)")+
  ylab("Effect of LUI differences on beta-diversity")+
  ggtitle("a - Land use intensity (LUI)")+
  #geom_text(aes(x = 3.5, y = 1.0, label = "EV% = 1.0"), color="black") + 
  #geom_text(aes(x = 3.5, y = 0.8, label = "EV% = 1.3"),color="black") + 
  #geom_text(aes(x = 3.5, y = 0.55, label = "EV% = 1.0"),color="black") + 
  #geom_text(aes(x = 3.5, y = 0.35, label = "EV% = 0.4"),color="black") +
  theme_classic()

p1

# Mowing
p2<- ggplot(splines_pred_yall_sc, aes(x=MOWx, y=MOWy, colour=beta_type))+
  geom_line(size=1.5)+
  scale_y_sqrt(limits = c(0, 3), expand = c(0, 0))+
  scale_colour_hue(name="Type of Beta Diversity", labels=c("Turnover", "Chao 0",
                                                           "Chao 1", "Chao 2",
                                                           "Chao 3", "Chao 4"),l=50)+
  xlab("MOW (scaled to EP average)")+
  ylab("Effect of MOW differences on beta-diversity")+
  ggtitle("b - Mowing intensity (MOW)")+
  #geom_text(aes(x = 3.3, y = 0.28, label = "EV% = 0.1"), color="black") + 
  #geom_text(aes(x = 3, y = 0.21, label = "EV% = 0.2"),color="black") + 
  #geom_text(aes(x = 3.5, y = 0.13, label = "EV% = 0.2"),color="black") + 
  #geom_text(aes(x = 3.5, y = 0.09, label = "EV% = 0.1"),color="black") + 
  theme_classic()
p2

# Grazing
p3<- ggplot(splines_pred_yall_sc, aes(x=GRAx, y=GRAy, colour=beta_type))+
  geom_line(size=1.5)+
  scale_y_sqrt(limits = c(0, 3), expand = c(0, 0))+
  scale_colour_hue(name="Type of Beta Diversity", labels=c("Turnover","Chao 0",
                                                           "Chao 1", "Chao 2",
                                                           "Chao 3", "Chao 4"
                                                           ),l=50)+
  xlab("GRA (scaled to EP average)")+
  ylab("Effect of GRA differences on beta-diversity")+
  ggtitle("c - Grazing intensity (GRA)")+
  #geom_text(aes(x = 10, y = 0.22, label = "EV% = 0.5"), color="black") + 
  #geom_text(aes(x = 10, y = 0.2, label = "EV% = 0.9"),color="black") + 
  #geom_text(aes(x = 10, y = 0.185, label = "EV% = 1.0"),color="black") + 
  #geom_text(aes(x = 10, y = 0.125, label = "EV% = 0.5"),color="black") + 
  theme_classic()

p3

# Fertilisation
p4<- ggplot(splines_pred_yall_sc, aes(x=FERx, y=FERy, colour=beta_type))+
  geom_line(size=1.5)+
  scale_y_sqrt(limits = c(0, 3), expand = c(0, 0))+
  scale_colour_hue(name="Type of Beta Diversity", labels=c("Turnover", "Chao 0",
                                                           "Chao 1", "Chao 2",
                                                           "Chao 3", "Chao 4"),l=50)+
  xlab("FER (scaled to EP average)")+
  ylab("Effect of FER differences on beta-diversity")+
  ggtitle("d - Fertilisation intensity (FER)")+
  #geom_text(aes(x = 10.5, y = 0.44, label = "EV% = 0.5"), color="black") + 
  #geom_text(aes(x = 10.5, y = 0.35, label = "EV% = 0.6"),color="black") + 
  #geom_text(aes(x = 10.5, y = 0.19, label = "EV% = 0.3"),color="black") +
  #geom_text(aes(x = 10.5, y = 0.12, label = "EV% = 0.2"),color="black") +
  theme_classic()

p4

p1 + p2 + p3 + p4 + plot_layout(guides="collect")
ggsave("ResultsPlots/pred_yall_LUsplines_scaled.png")
