...
for(k in 1:3){
  ...
  for(l in 1:6){
    sub<- scaled_space[[k]]
    lm<- lm(as.formula(paste(colnames(sub[l]),"~mFER+dFER+dph+mph+msur+dsur+dsoilPC1+msoilPC1+geo.dist+YR",sep="")),data=sub)
    lm1<- update(lm,~.-mFER)
    lm2<- update(lm,~.-dFER)
    lm3<- update(lm1,~.-dFER)
    ...
    m<- (summary(lm)$r.squared-summary(lm1)$r.squared)#variance expl by mFER
    d<- (summary(lm)$r.squared-summary(lm2)$r.squared)#variance expl by dFER
  ...
  EV_space[[k]]<- data.frame(all, mFERpp, dFERpp, sharedpp, cofm, cofd, int, pvm, pvd)
}
...
for(k in 1:3){
  ...
  for(l in 1:6){
    sub<- scaled_time[[k]]
    lm<- lm(as.formula(paste(colnames(sub[l]),"~mFER+dFER+pH+G500+soilPCA+HWG+RWG+dYR",sep="")),data=sub)
    lm1<- update(lm,~.-mFER)
    lm2<- update(lm,~.-dFER)
    lm3<- update(lm1,~.-dFER)
  ...
  EV_time[[k]]<- data.frame(all, mFERpp, dFERpp, sharedpp, cofm, cofd, int, pvm, pvd)
}
